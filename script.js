    ...
    <script>
        ...
        function queryCFValue(clothNo) {
            ...
            if (found) {
                ...
                // 如果已有输入值，重新计算
                if (lengthInput.value && parseNumber(lengthInput.value) > 0) {
                    calculateFromLength();
                } else if (weightInput.value && parseNumber(weightInput.value) > 0) {
                    calculateFromWeight();
                }
            } else {
                cfValueEl.textContent = '未查询到';
        ...
        function addCloth() {
            ...
            if (clothNoInput.value.toUpperCase() === clothNo.toUpperCase()) {
                ...
                if (lengthInput.value) calculateFromLength();
                if (weightInput.value) calculateFromWeight();
            }
        }
            ...
            clothData.forEach((item, index) => {
                ...
                row.innerHTML = `
                    ...
                    <td class="px-3 py-2 text-center">
                        <button onclick="deleteCloth(${index})" class="text-red-500 hover:text-red-700 px-2 py-1 rounded">
