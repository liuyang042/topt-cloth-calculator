// 页面元素获取
    ...
    function queryCFValue(clothNo) {
      ...
      if (found) {
        // 优化数据显示：保留5位小数
        cfValueEl.textContent = found.cf.toFixed(5);
    ...
    calculateBtn.addEventListener('click', () => {
      if (lengthInput.value && weightInput.value) {
        resultEl.textContent = '请只输入米长或重量中的一个值进行计算';
        return;
      }
      if (lengthInput.value) calculate('length');
      if (weightInput.value) calculate('weight');
    ...
    function calculate(type) {
      ...
      if (type === 'length' && length > 0) {
        ...
        resultEl.innerHTML = `计算完成：${length}M ÷ ${cf.toFixed(5)} = <span class="font-bold text-primary">${Math.round(calcWeight)}KG</span>`;
      } else if (type === 'weight' && weight > 0) {
        const calcLength = weight * cf;
        lengthInput.value = Math.round(calcLength); // 取整
        resultEl.innerHTML = `计算完成：${weight}KG × ${cf.toFixed(5)} = <span class="font-bold text-primary">${Math.round(calcLength)}M</span>`;
    ...
    function renderClothList() {
      ...
      clothData.forEach((item, index) => {
        ...
        row.innerHTML = `
          <td class="px-3 py-2 font-medium">${item.clothNo}</td>
          <td class="px-3 py-2">${item.cf.toFixed(5)}</td>
    ...
    exportBtn.addEventListener('click', () => {
      const dataStr = JSON.stringify(clothData, null, 2);
      const dataBlob = new Blob([dataStr], {type: 'application/json'});
      
      const url = URL.createObjectURL(dataBlob);
      const link = document.createElement('a');
      link.href = url;
      link.download = `TOPT布号配置_${new Date().toISOString().split('T')[0]}.json`;
      ...
      reader.onload = function(e) {
        try {
          ...
          for (let item of importedData) {
            if (!item.clothNo || typeof item.clothNo !== 'string' || 
                !item.cf || typeof item.cf !== 'number') {
              alert('导入的文件数据格式不正确');
              return;
            }
          }
          ...
          // 合并数据（去重）
          const existingClothNos = clothData.map(item => item.clothNo.toUpperCase());
          const newItems = importedData.filter(item => 
            !existingClothNos.includes(item.clothNo.toUpperCase())
          );
          
          clothData = [...clothData, ...newItems];
          renderClothList();
